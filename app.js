(() => {
  ...
  function panToPoint(lat, lon) {
    ...
    const headerH = topbarEl ? topbarEl.getBoundingClientRect().height : 60;
    const panelH = panelClearance();

    // Vertical band that's actually free (not covered by header or panel).
    const bandTop = headerH + 20;
    const bandBottom = Math.max(bandTop + 40, size.y - panelH - 20);
    const desiredY = (bandTop + bandBottom) / 2;

    // Find what new map center would put this point at (cx, desiredY)
    // instead of dead-center, using exact pixel math (no guessed padding).
    const shifted = L.point(cx, 2 * cy - desiredY);
    const newCenter = map.containerPointToLatLng(shifted);
    map.setView(newCenter, targetZoom, { animate: true, duration: 0.5 });
  }

  // Re-position the map for whatever is currently shown (route, or a single
  // pickup/drop pin) using the panel's *current* height. Needed whenever the
  // panel's collapsed/expanded state changes after a pin/route was already
  // positioned for a different panel height — otherwise a pin can end up
  // hidden underneath the panel.
  function reflowMapForPanel() {
    if (lastRoute && routeLine) {
      ...
      panToPoint(pickup.lat, pickup.lon);
    } else if (drop && !pickup) {
      panToPoint(drop.lat, drop.lon);
    }
  }
  ...
  function initPicking() {
    ...
    if (btnPanelToggle) {
      btnPanelToggle.addEventListener("click", function () {
        ...
        updatePanelCollapse();
        setTimeout(reflowMapForPanel, 340);
      });
    }
  }
  ...
  function initInstall() {
    ...
    btnInstall.addEventListener("click", async function () {
      if (!deferredInstallPrompt) {
        showRule("Install isn't available right now — try reloading the page, or use your browser's own \"Add to Home Screen\" option.");
        return;
      }
      ...
      try {
        ...
        if (choice.outcome === "accepted") {
          ...
          // Declined — Chrome won't reliably refire beforeinstallprompt soon,
          // so keep the button around but let them know why nothing happened.
          showRule("Install was cancelled. Tap Install again anytime to retry.");
        }
      } catch (err) {
        console.error("Install prompt failed:", err);
        showRule("Couldn't open the install prompt. If you're viewing this inside another app (like WhatsApp's built-in browser), open the site in Chrome directly and try again.");
      }
    });
