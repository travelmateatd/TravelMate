...
/**
 * ✏️ MAJOR CITIES — round-trip-fare exemption list
 * Routes ending in these cities are charged ONE-WAY fare because
 * the driver reliably finds a return booking from there.
 * All other destinations are charged DOUBLE fare (round trip)
 * since the driver returns to Abbottabad empty.
 * Add/remove city names below (case-insensitive, partial match).
 */
const MAJOR_CITIES = ['islamabad', 'rawalpindi', 'peshawar', 'lahore'];
...
/* Check if drop-off text matches any major city (case-insensitive partial match) */
function isMajorCityRoute() {
  var dropoffEl = document.getElementById('dropoff');
  if (!dropoffEl) return true; // default: don't double if unknown
  var val = dropoffEl.value.trim().toLowerCase();
  if (!val) return true;
  for (var i = 0; i < MAJOR_CITIES.length; i++) {
    if (val.indexOf(MAJOR_CITIES[i]) !== -1) return true;
  }
  return false;
}
...
function recalcFare() {
  ...
  var isMajor   = isMajorCityRoute();
  var baseTotal = (km > 0 && rate > 0) ? km * rate : 0;
  var total     = isMajor ? baseTotal : baseTotal * 2;
  ...
  if (sumKm)   sumKm.textContent   = km > 0 ? km + ' km' + (!isMajor && km > 0 ? ' × 2 (round trip)' : '') : '—';
  if (sumRate) sumRate.textContent = rate > 0 ? rate + ' PKR' : '—';

  // Show/hide round-trip notice
  if (rtNotice) {
    if (km > 0 && !isMajor) {
      rtNotice.style.display = 'flex';
    } else {
      rtNotice.style.display = 'none';
    }
  }
...
function handleWhatsappBooking() {
  ...
  var rate     = selectedCar.rate;
  var isMajor  = isMajorCityRoute();
  var baseTotal = km * rate;
  var total    = isMajor ? baseTotal : baseTotal * 2;
  ...
  var fareLine = isMajor
    ? 'Total Price: ' + total.toLocaleString('en-PK') + ' PKR'
    : 'Fare Type: Round Trip (return journey included)\n'
  ...
  var message = 'Hello, I want to book a taxi.\n\n'
    + 'Customer Name: ' + name + '\n'
    + 'Phone: ' + phone + '\n'
    + 'Car: ' + selectedCar.name + ' (' + rate + ' PKR/km)\n'
    + 'Pickup: ' + pickup + '\n'
    + 'Drop-off: ' + dropoff + '\n'
    + 'Date: ' + formattedDate + '\n'
